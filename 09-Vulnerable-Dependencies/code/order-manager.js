...
exports.handler = (event, context, callback) => {
    ...
    var headers = JSON.parse(typeof event.headers === 'string' ? event.headers : JSON.stringify(event.headers));
    var auth_header = headers.Authorization || headers.authorization;
    var token_sections = auth_header.split('.');
    // var auth_data = jose.util.base64url.decode(token_sections[1]);
    var auth_data = base64url.decode(token_sections[1]);
